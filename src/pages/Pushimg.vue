<template>
  <div class="pushimg_page">
    <div class="input">
      <input type="textarea" autofocus placeholder="请输入文章标题" v-model="sendnote.title" @focus="islook = false" maxlength="20">
      ...
      <imglode ref="selection" style="padding-bottom: 0.2rem;"  v-on:getImg="getImg"></imglode>
    </div>
...
export default {
  ...
  methods: {
    ...
    getImg(data){
      if(this.sendnote.img == ''){
        this.sendnote.img = new Array()
      }
      if(this.sendnote.uurl == ''){
        this.sendnote.uurl = new Array()
      }
      console.log(data)
      this.sendnote.img.push(data.id)
      this.sendnote.uurl.push(data.url)
    }
  },
  mounted(){
